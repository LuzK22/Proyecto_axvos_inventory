...
@section('title', 'Préstamo #' . $loan->id . ' — Otros Activos')

@section('content_header')
<div class="d-flex justify-content-between align-items-center">
    <h1 class="m-0">
        <i class="fas fa-handshake mr-2" style="color:#7c3aed;"></i>
        Préstamo #{{ $loan->id }}
    ...
    <div>
        @if($loan->status !== 'devuelto')
            <a href="{{ route('assets.loans.return', $loan) }}" class="btn btn-sm btn-warning mr-1">
                <i class="fas fa-undo mr-1"></i> Registrar Devolución
            </a>
        @endif
        <a href="{{ route('assets.loans.index') }}" class="btn btn-sm btn-secondary">
...
<div class="row">
    <div class="col-md-6">
        {{-- Activo --}}
        <div class="card shadow-sm mb-3" style="border-left:4px solid #7c3aed;">
            <div class="card-header py-2">
                <h6 class="mb-0 font-weight-bold">
                    <i class="fas fa-box mr-1" style="color:#7c3aed;"></i> Activo
                </h6>
            </div>
            <div class="card-body py-2 px-3">
                ...
                <div class="mt-2">
                    <a href="{{ route('assets.show', $loan->asset) }}" class="btn btn-xs btn-outline-primary">
            ...
            <div class="card-header py-2">
                <h6 class="mb-0 font-weight-bold">
                    @if($loan->destination_type === 'branch')
                        <i class="fas fa-building mr-1" style="color:#059669;"></i> Sucursal Destino
                    @else
                        <i class="fas fa-user mr-1" style="color:#059669;"></i> Colaborador
                    @endif
                </h6>
            </div>
            <div class="card-body py-2 px-3">
                @if($loan->destination_type === 'branch')
                    <table class="table table-sm table-borderless mb-0">
                        <tr><td class="text-muted pl-0" style="width:35%;font-size:.82rem;">Sucursal</td>
                            <td class="font-weight-bold">{{ $loan->destinationBranch?->name ?? '—' }}</td></tr>
                        <tr><td class="text-muted pl-0" style="font-size:.82rem;">Tipo destino</td>
                            <td><span class="badge" style="background:#7c3aed;color:#fff;font-size:.7rem;">Sucursal</span></td></tr>
                    </table>
                @else
                    <table class="table table-sm table-borderless mb-0">
                        <tr><td class="text-muted pl-0" style="width:35%;font-size:.82rem;">Nombre</td>
                            <td class="font-weight-bold">{{ $loan->collaborator?->full_name ?? '—' }}</td></tr>
                        <tr><td class="text-muted pl-0" style="font-size:.82rem;">Cédula</td>
                            <td>{{ $loan->collaborator?->document ?? '—' }}</td></tr>
                        <tr><td class="text-muted pl-0" style="font-size:.82rem;">Cargo</td>
                            <td>{{ $loan->collaborator?->position ?? '—' }}</td></tr>
                        <tr><td class="text-muted pl-0" style="font-size:.82rem;">Sucursal</td>
                            <td>{{ $loan->collaborator?->branch?->name ?? '—' }}</td></tr>
                        @if($loan->collaborator?->email)
                        <tr><td class="text-muted pl-0" style="font-size:.82rem;">Correo</td>
                            <td><small>{{ $loan->collaborator->email }}</small></td></tr>
                        @endif
                    </table>
                @endif
            </div>
    ...
    <div class="col-md-6">
        ...
        <div class="card shadow-sm mb-3" style="border-left:4px solid #f59e0b;">
            <div class="card-header py-2">
                <h6 class="mb-0 font-weight-bold">
                    <i class="fas fa-calendar mr-1" style="color:#f59e0b;"></i> Fechas
                </h6>
            </div>
            <div class="card-body py-2 px-3">
                <table class="table table-sm table-borderless mb-0">
                    <tr><td class="text-muted pl-0" style="width:40%;font-size:.82rem;">Inicio</td>
                        <td>{{ $loan->start_date->format('d/m/Y') }}</td></tr>
                    <tr><td class="text-muted pl-0" style="font-size:.82rem;">Devolución pactada</td>
                        <td class="{{ $loan->status==='vencido'?'text-danger font-weight-bold':'' }}">
        ...
        <div class="card shadow-sm mb-3" style="border-left:4px solid #6c757d;">
            <div class="card-header py-2">
                <h6 class="mb-0 font-weight-bold">
                    <i class="fas fa-info-circle mr-1 text-muted"></i> Registro
                </h6>
            </div>
            <div class="card-body py-2 px-3">
                <table class="table table-sm table-borderless mb-0">
                    <tr><td class="text-muted pl-0" style="width:40%;font-size:.82rem;">Registrado por</td>
                        <td>{{ $loan->creator?->name ?? '—' }}</td></tr>
                    <tr><td class="text-muted pl-0" style="font-size:.82rem;">Fecha registro</td>
                        <td>{{ $loan->created_at->format('d/m/Y H:i') }}</td></tr>
                    @if($loan->returnedBy)
                    <tr><td class="text-muted pl-0" style="font-size:.82rem;">Devolución por</td>
                        <td>{{ $loan->returnedBy->name }}</td></tr>
                    @endif
                </table>
                @if($loan->notes)
